...
// ==============================
// NORMALIZAR TEXTO (soporta errores comunes)
// ==============================
...
// ==============================
// FUNCIONES DE ACCIÓN (enviar mensajes, imágenes, catálogo)
// ==============================
...
// ==============================
// IA CENTRAL CON DECISIÓN ESTRUCTURADA (JSON)
// ==============================
async function procesarConIA(textoUsuario, from) {
  // Construir un prompt detallado con el catálogo real
  const catalogoInfo = Object.entries(catalogo)
    .map(([id, prod]) => `- id: ${id}, nombre: ${prod.nombre}${prod.tonos ? `, tonos: ${prod.tonos.join(", ")}` : ""}${prod.rendimiento ? `, rendimiento: ${prod.rendimiento} unidades/m2` : ""}`)
    .join("\n");
...
- "producto_id": string (solo si accion es "enviar_imagenes", debe coincidir con alguna de las ids del catálogo).

Reglas importantes:
- Si el usuario pide ver el catálogo ("qué productos tienes", "muéstrame todo", "qué manejas", etc.) → accion="enviar_catalogo".
- Si el usuario pide fotos de un producto específico (ej: "enséñame el adoquín 20x10x3", "quiero ver la fachaleta cappuccino", "muéstrame el adoquin ecológico") → accion="enviar_imagenes" y producto_id debe coincidir exactamente con la id del catálogo.
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    // Inicializar sesión si no existe (por si necesitas historial en el futuro)
    if (!sessions.has(from)) {
      sessions.set(from, { history: [] });
    }

    // Procesar el mensaje con la IA central
